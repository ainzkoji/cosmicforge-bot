    ...
    def _get_loss_streak_from_db(self, symbol: str) -> int:
        ...
        try:
            with self.db.connect() as conn:
                scoped = self._has_column(conn, "trade_fills", "bot_instance_id")
                sql = """
                    SELECT realized_pnl
                    FROM trade_fills
                    WHERE symbol = ?
                ...
                params = [symbol]
                if scoped:
                    sql += " AND (bot_instance_id = ? OR (bot_instance_id IS NULL AND ? = 'default'))"
                    params.extend([self.bot_instance_id, self.bot_instance_id])
                rows = conn.execute(sql + " ORDER BY timestamp_utc DESC LIMIT 30", params).fetchall()
    ...
    def _get_drawdown_from_db(self, user_id: Optional[str] = None) -> float:
        ...
        try:
            with self.db.connect() as conn:
                ...
                scoped = self._has_column(conn, "equity_snapshots", "bot_instance_id")
                time_column = "timestamp_utc" if self._has_column(conn, "equity_snapshots", "timestamp_utc") else "snapshot_time"
                sql = "SELECT equity FROM equity_snapshots"
                params = []
                if scoped:
                    sql += " WHERE (bot_instance_id = ? OR (bot_instance_id IS NULL AND ? = 'default'))"
                    params.extend([self.bot_instance_id, self.bot_instance_id])
                rows = conn.execute(sql + f" ORDER BY {time_column} ASC LIMIT 500", params).fetchall()
    ...
    def _get_rolling_stats_from_db(self, symbol: str, lookback: int = 20) -> tuple:
        ...
        try:
            with self.db.connect() as conn:
                ...
                rows = conn.execute(sql + " ORDER BY timestamp_utc DESC LIMIT ?", params).fetchall()
