...
def evaluate_user_capital_readiness(
    *,
    ...
    from app.product_safety.approvals import sections_a_to_e_confirmed
    sections_confirmed = bool(getattr(settings, "SECTIONS_A_TO_E_CONFIRMED", False)) or sections_a_to_e_confirmed(db=db)
    ...
    if status == ReadinessStatus.READY_FOR_CONTROLLED_BETA_REVIEW:
        from app.product_safety.approvals import active_readiness_approval
        if active_readiness_approval(db=db, bot_instance_id=bot_instance_id):
            status = ReadinessStatus.APPROVED_FOR_CONTROLLED_BETA
...
def assert_user_capital_activation_allowed(*, db: DB, bot_instance_id: str) -> None:
    from app.product_safety.approvals import active_readiness_approval, invalidate_readiness_approval
    approval = active_readiness_approval(db=db, bot_instance_id=bot_instance_id)
    if approval:
        try:
            from app.api.readiness_admin import _current_policy_hash
            current_hash = _current_policy_hash(db, bot_instance_id)
            if approval.get("policy_hash") != current_hash:
                invalidate_readiness_approval(
                    db=db, bot_instance_id=bot_instance_id,
                    reason="MATERIAL_POLICY_CHANGE", current_policy_hash=current_hash,
                )
        except Exception:
            # Any inability to prove policy identity leaves activation fail-closed.
            invalidate_readiness_approval(
                db=db, bot_instance_id=bot_instance_id,
                reason="POLICY_REVALIDATION_FAILED", current_policy_hash=None,
            )
