...
class TradingOrchestrator:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.effective_policy = kwargs.get("effective_policy")
    ...
    def _create_risk_budget_config(self) -> RiskBudgetConfig:
        ...
        max_risk_per_trade = (
            float(self.effective_policy.risk_per_trade)
            if self.effective_policy is not None
            else self.config_validator.get_risk_per_trade_limit(self.validated_config.risk_level)
        )
        ...
        **kwargs
        ...
        if not gate_decision.allowed:
            ...
            self.record_decision(
                ...
                risk_gate_decision_json=json.dumps({
                    **result["details"]["observability"],
                    "reason_code": getattr(gate_decision.block_reason, "value", str(gate_decision.block_reason)),
                    "reason_detail": gate_decision.message,
                    "gate_details": gate_decision.details,
                }),
                sizing_decision_json="{}",
                protection_decision_json="{}",
                final_action="blocked"
        ...
        ctx = PolicyContext(
            ...
            max_weekly_drawdown_pct=float(kwargs.get("max_weekly_drawdown_pct", 0.0)),
            max_monthly_drawdown_pct=float(kwargs.get("max_monthly_drawdown_pct", 0.0)),
            min_risk_reward=float(kwargs.get("min_risk_reward", 0.0)),
            stop_loss_price=policy_stop,
            take_profit_price=policy_take_profit,
        )
        ...
        if not decision.allowed:
            ...
            self.record_decision(
                ...
                strategy_signal_json=json.dumps(result["details"].get("strategy_output", {})),
                risk_gate_decision_json=json.dumps(result["details"]["observability"]),
                sizing_decision_json=json.dumps({
                    "sizing_evaluated": True,
                    "allowed": False,
