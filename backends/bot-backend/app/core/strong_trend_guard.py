...
class StrongTrendGuardStatus:
    ...
    forced_blocked: bool
    monitoring_stop_recommended: bool
...
def evaluate_strong_trend_guard(
    ...
    *,
    ...
    monitoring_stop_recommended = False
    report_path = str(getattr(config, "STRONG_TREND_MONITOR_REPORT_PATH", "") or "").strip()
    if report_path:
        try:
            report = json.loads(Path(report_path).read_text(encoding="utf-8"))
            monitoring_stop_recommended = bool(report.get("stop_recommended", False))
        except FileNotFoundError:
            pass
        except Exception:
            # A malformed control report must not silently unblock an experiment
            # that configuration already blocks; normal configured safety remains.
            pass
    ...
    forced_blocked = configured_unblocked and (
        not safety_requirements_met or monitoring_stop_recommended
    )
    if forced_blocked:
    ...
    elif monitoring_stop_recommended:
        reason = "monitoring_stop_recommended"
    elif forced_blocked:
    ...
    return StrongTrendGuardStatus(
        ...
        monitoring_stop_recommended=monitoring_stop_recommended,
