...
class SystemLimits:
    ...
    max_risk_per_trade_ceiling: float = 0.004  # 0.4% = ~$120 on $3000 account
...
class UserConfigurableLimits:
    ...
    min_strategy_confidence: float = 0.10  # Lowered from 0.5 to allow more trades
...
class ConfigValidator:
    ...
    def _get_max_leverage_for_asset(self, asset_class: AssetClass) -> float:
        """Get max leverage for asset class."""
        if asset_class == AssetClass.MAJOR_CRYPTO:
            return self.limits.max_leverage_major
        if asset_class == AssetClass.ALT_CRYPTO:
            return self.limits.max_leverage_alt
        if asset_class == AssetClass.MEME_CRYPTO:
            return self.limits.max_leverage_meme
        if asset_class == AssetClass.STABLE_PAIRS:
            return self.limits.max_leverage_stable
        return self.limits.max_leverage_alt
