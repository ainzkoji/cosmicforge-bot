...
class RiskBudgetEngine:
    ...
    def can_add_position(
        ...
        # =========================================================
        # 3) Maintenance Margin Modeling (Liquidation Safety)
        # =========================================================
        # We estimate maintenance margin dynamically. In crypto, it's typically ~0.4% - 0.5% 
        # of notional value for smaller tiers. We use 0.5% as a conservative safe bound.
        # Liquidation occurs when (Equity) < (Maintenance Margin).
        # We enforce a hard ceiling where total Maintenance Margin cannot exceed 50% of Equity.
        
        MAINTENANCE_MARGIN_RATE = 0.005  # 0.5% of notional
        current_maint_margin_est = sum(p.notional * MAINTENANCE_MARGIN_RATE for p in self._positions)
        new_maint_margin_est = current_maint_margin_est + (notional * MAINTENANCE_MARGIN_RATE)
        
        max_maint_margin_allowed = equity * 0.50  # Hard floor for liquidation safety
        
        if new_maint_margin_est > max_maint_margin_allowed:
            return BudgetDecision(
                False,
                "MAINTENANCE_MARGIN_CRITICAL",
                f"Est. Maint Margin {new_maint_margin_est:.2f} > max allowed {max_maint_margin_allowed:.2f}. Blocked to prevent liquidation cascade.",
                "HARD_BLOCK",
                "MARGIN_LEVEL",
                new_maint_margin_est,
                max_maint_margin_allowed,
                0
            )
...
# Each bot instance has its own risk budget so one bot's open positions and
# risk consumption never block or contaminate another bot's budget checks.

import threading as _threading

_budget_engines: dict[str, "RiskBudgetEngine"] = {}
_engines_lock = _threading.Lock()
...
def get_risk_budget_engine(
    ...
    """Return the RiskBudgetEngine for *bot_id*, creating it on first call."""
    with _engines_lock:
        if bot_id not in _budget_engines:
            _budget_engines[bot_id] = RiskBudgetEngine(config or RiskBudgetConfig())
        return _budget_engines[bot_id]


def reset_risk_budget_engine(bot_id: str = "default") -> None:
    """Evict a single bot's budget engine (e.g. on bot teardown or test reset)."""
    with _engines_lock:
        _budget_engines.pop(bot_id, None)


def reset_all_risk_budget_engines() -> None:
