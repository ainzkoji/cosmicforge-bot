...
class ExchangeCircuitBreaker:
    ...
    def record_error(self, error: Exception = None):
        # Only trip circuit breaker for ExchangeError types, ignore logic/fatal bugs
        if error is not None and type(error).__name__ not in ("ExchangeError", "Exception"):
            # If a strongly typed error that is NOT ExchangeError is passed, don't trip
            pass 
        else:
            now = time.time()
