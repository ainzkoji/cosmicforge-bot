...
class CreateBotInstanceRequest:
    ...
    def validate(self) -> List[str]:
        ...
        if self.strategy_id == "master_ensemble" and (
            self.capital_allocation is None or self.capital_allocation <= 0
        ):
            errors.append("capital_allocation is required for Auto Pilot")
        if (
            self.capital_allocation_type == "fixed_amount"
            and self.allocation_type == "fixed_amount"
            and self.capital_allocation is not None
            and self.allocation_value > self.capital_allocation
        ):
            errors.append("allocation_value cannot exceed capital_allocation")
