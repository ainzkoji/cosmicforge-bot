...
if db_path:
    ...
    if resolved_db != abs_db_path:
        raise RuntimeError(
            f"FATAL: DB Split detected! DATABASE_URL={abs_db_path} but DB() resolved to {resolved_db}"
        )

    resolved_trace = os.path.abspath(get_trace_recorder()._db_path)
    if resolved_trace != abs_db_path:
        raise RuntimeError(
            f"FATAL: Trace DB Split! Expected {abs_db_path} but got {resolved_trace}"
        )

# Run migrations on the CORRECT database
migrate(db_path=db_path)

# Make the active evidence source explicit without moving or deleting any file.
try:
    _active_db = Path(DB().path).resolve()
    _stat = _active_db.stat() if _active_db.exists() else None
    with DB().connect() as _diag_conn:
        _schema_version = int(_diag_conn.execute("PRAGMA user_version").fetchone()[0])
    _candidate_roots = {_active_db.parent, Path(__file__).resolve().parents[2] / "data"}
    _candidates = sorted({p.resolve() for root in _candidate_roots if root.exists() for p in root.glob("*.db")})
    print(
        "[DATABASE_EVIDENCE] "
...
except Exception as _db_diag_exc:
    print(f"[DATABASE_EVIDENCE_WARNING] diagnostics_failed={_db_diag_exc}")

# [RUNTIME_BASELINE] — bind this process to a code revision, interpreter and DB file
# so that every piece of trading evidence can be traced back to what produced it.
try:
    from app.ops.runtime_baseline import emit_runtime_baseline
    from app.runner.effective_policy import normalize_execution_mode

    RUNTIME_BASELINE = emit_runtime_baseline(
        db_path=DB().path,
...
try:
    ...
    print(
        f"[SSE] Server-Sent Events disabled (missing dependency: {e}). Analytics will use polling only."
    )
...
app.include_router(
    ...
    prefix="/api/v1/news-intelligence",
...
app.include_router(
    tradingview_router, prefix="/api/v1/tradingview", tags=["TradingView"]
)
...
async def _startup_validate_config():
    ...
    try:
        ...
        print(
            f"[STARTUP] Safety check: {'PASS' if safety_report.all_pass else 'WARNINGS (paper mode continues)'}"
        )
    except RuntimeError:
        raise
    except Exception as e:
...
class RunnerServiceState:
    ...
    multi_runner: Optional[Any] = (
        None  # Avoid circular import if needed, or use Type Hint
    )
...
calendar_sync_worker = build_calendar_sync_worker(
    _worker_db, calendar_service=_event_calendar_svc
)
...
async def _startup_signal_scheduler():
    ...
    try:
        ...
        print(
            f"[SIGNAL_SCHEDULER] WARNING: Could not import scheduler dependencies: {exc}"
        )
        return
    ...
    print(
        f"[SIGNAL_SCHEDULER] SIGNAL_SCHEDULER_STARTED generation_times_utc={SIGNAL_GENERATION_TIMES_UTC}"
    )
...
def trade_market(symbol: str = "XRPUSDT", side: str = "BUY", usdt: float = 10.0):
    ...
    """
    raise HTTPException(
        status_code=410, detail="Manual trading disabled. Use Auto Pilot."
    )
...
def risk_status():
    ...
    risk_state.open_positions = sum(
        1 for s in runner.state.values() if s.position in ("LONG", "SHORT")
    )
    ...
    return {
        ...
        "settings": {
            ...
            "max_weekly_drawdown_pct": getattr(
                settings, "MAX_WEEKLY_DRAWDOWN_PCT", 0.0
            ),
            "max_monthly_drawdown_pct": getattr(
                settings, "MAX_MONTHLY_DRAWDOWN_PCT", 0.0
            ),
        },
    }
...
def runner_status() -> dict:
    ...
    with db.connect() as conn:
        rows = conn.execute(
            """
            SELECT id, user_id, broker_account_id, market_type, strategy_id,
                   symbols_json, timeframes_json, mode, status, last_run_at,
        ...
        for row in rows:
            item = dict(row)
            item["signal_source"] = "internal_strategy"
            item["signal_center"] = "advisory_non_executable"
            item["external_signal_queue_pending"] = conn.execute(
...
def trade_close_record_usertrades(symbol: str = "ETHUSDT", window_minutes: int = 10):
    ...
    with runner.symbol_guard(symbol, timeout_s=2.0) as ok:
        ...
        _close_position_id = getattr(
            st, "position_id", _row_get(open_meta, "position_id", None)
        )
        ...
        record_fill(
            ...
            broker_account_id=(
                broker_account_id if "broker_account_id" in locals() else None
            ),
        )
...
def _csv_runtime(value: str) -> list[str]:
    return [
        part.strip().upper() for part in str(value or "").split(",") if part.strip()
    ]
...
def _phase6_runtime_fingerprint() -> Dict[str, Any]:
    ...
    try:
        ...
        if row:
            lockout_status = get_tradingview_safety_lockout(
                DB(path=get_db_path()), row["id"]
            )
    except Exception as exc:
    ...
    return {
        ...
        "TRADINGVIEW_ALLOWED_SYMBOLS": _csv_runtime(
            settings.TRADINGVIEW_ALLOWED_SYMBOLS
        ),
        "TRADINGVIEW_ALLOWED_ACTIONS": _csv_runtime(
            settings.TRADINGVIEW_ALLOWED_ACTIONS
        ),
        ...
        "active_safety_lockout": bool(
            lockout_status and int(lockout_status.get("is_locked") or 0)
        ),
...
async def tradingview_limited_status():
    ...
    return {
        ...
        "auto_disable_on_invariant_fail": fp[
            "TRADINGVIEW_AUTO_DISABLE_ON_INVARIANT_FAIL"
        ],
...
async def health():
    ...
    return {
        ...
        "trade_symbols_scope": "legacy_contextless_development_replay",
        "universe_default_mode": settings.UNIVERSE_DEFAULT_MODE,
        "universe_active_limit": settings.UNIVERSE_ACTIVE_LIMIT,
...
def _sanity_checks() -> Dict[str, Any]:
    ...
    if settings.STOP_LOSS_PCT > 0.05:
        warnings.append(
            f"STOP_LOSS_PCT ({settings.STOP_LOSS_PCT:.2%}) is high for leveraged trading."
        )
...
def debug_check_symbol(symbol: str = "ETHUSDT"):
    ...
    klines = []
    try:
        klines = runner.client.klines(
            symbol.upper(), interval=settings.DEFAULT_INTERVAL, limit=5
        )
    except Exception as e:
        klines = [str(e)]
