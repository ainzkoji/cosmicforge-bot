...
class RegimeClassifier:
    ...
    def classify_stable(
        ...
        if self._last_regime is None:
            ...
            self._candidate_regime = result.regime
            self._regime_count = 1
            return result

        if result.regime == self._last_regime:
            self._candidate_regime = result.regime
            self._regime_count = self.thresholds.stability_candles
            return result

        if result.regime == self._candidate_regime:
            self._regime_count += 1
        else:
            self._candidate_regime = result.regime
            self._regime_count = 1

        if self._regime_count >= self.thresholds.stability_candles:
            self._last_regime = result.regime
            return result
        ...
        return RegimeResult(
            ...
            regime_confidence=result.regime_confidence * 0.8,
            adx=result.adx,
            ...
            details={
                **result.details,
                "stabilizing": True,
                "candidate_regime": result.regime.value,
                "count": self._regime_count,
            },
        )
