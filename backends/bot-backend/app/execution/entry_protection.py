...
class EntryProtection:
    ...
    def reconcile_entry(
        ...
        if (
            evidence == "FILLED"
            and str(entry.get("submit_state") or "") == SubmitState.SUBMIT_UNKNOWN.value
        ):
            # A just-observed fill is stronger than a transient flat position read.
            # Hold the lock for this pass; later passes use broker position truth
            # and the normal multi-confirmation flat release.
            with self.db.connect() as conn:
                conn.execute(
                    """UPDATE pending_entries SET submit_state=?, last_reconcile_at_ms=?, updated_at=datetime('now')
                       WHERE bot_id=? AND symbol=? AND side=?""",
                    (SubmitState.SUBMIT_CONFIRMED.value, now_ms, bot_id, symbol_up, side_up),
                )
                ...
                if held_entry:
                    self._append_event(
                        ...
                        conn=conn,
                        entry=held_entry,
                        reason="filled_order_waiting_for_position_truth",
                    )
