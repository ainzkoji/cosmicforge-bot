...
class PositionManager:
    ...
    def open_position(
        ...
        *args,
        ...
        """
        # Backward-compatible positional contract:
        #   (entry_price, qty, stop, tp1, tp2)
        # The newer position_id is keyword-only so it cannot shift quantity fields.
        if args:
            if len(args) != 5 or any(v is not None for v in (entry_price, qty, stop_price, tp1_price, tp2_price)):
                raise TypeError("open_position expects five lifecycle price/quantity positional arguments")
            entry_price, qty, stop_price, tp1_price, tp2_price = args
        if any(v is None for v in (entry_price, qty, stop_price, tp1_price, tp2_price)):
            raise TypeError("entry_price, qty, stop_price, tp1_price and tp2_price are required")
