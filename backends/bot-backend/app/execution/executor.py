...
def _apply_sl_tp_rounding_with_buffer(
    ...
    return float(sl_rounded), float(tp_rounded)
...
class BinanceExecutor:
    ...
    def _execute_impl(
        ...
        if effective_mode != "live":
            if signal == "CLOSE":
                paper = self.paper_executor.close_position(
                    symbol=symbol,
    ...
    def execute_tp1_partial_close(
        ...
        if effective_mode != "live":
            ...
            paper_partial = self.paper_executor.partial_close(
                symbol=symbol,
                position_side=pos_side_str,
                quantity=norm_qty,
            )
            if not paper_partial.success:
                result_base["failure_reason"] = paper_partial.error or "PAPER_PARTIAL_CLOSE_FAILED"
                return result_base
            runner_qty = float(paper_partial.details.get("remaining_qty") or 0.0)
            result_base.update(
                {
                    ...
                    "broker_response": paper_partial.details,
                    "fill_id": paper_partial.details.get("fill_id"),
                    "fill_price": paper_partial.avg_price,
                    "fee": paper_partial.fee,
                    "realized_pnl": paper_partial.details.get("realized_pnl", 0.0),
    ...
    def _normalize_be_stop_price(self, symbol: str, price: float, position_side: str) -> tuple[float, str]:
        """Normalize a BE stop without moving it toward the losing side."""
        from decimal import Decimal, ROUND_CEILING, ROUND_FLOOR
        try:
            filters = self.client.get_symbol_filters(symbol)
            tick = Decimal(str(filters.tick_size))
            value = Decimal(str(price))
            rounding = ROUND_CEILING if str(position_side).upper() == "LONG" else ROUND_FLOOR
            normalized = (value / tick).to_integral_value(rounding=rounding) * tick
            return float(normalized), "OK"
        except Exception:
            return float(price), "FILTERS_UNAVAILABLE"

    def _normalize_trailing_stop_price(
        self, symbol: str, price: float, position_side: str
    ) -> tuple[float, str]:
        """Normalize a trailing stop toward the protected/profitable side."""
        return self._normalize_be_stop_price(symbol, price, position_side)
    ...
    def ensure_protection(
        ...
        if has_sl and has_tp:
            ...
            if entry_price > 0 and not self._protection_is_sane(
                ...
                try:
                    ...
                    self.tpsl_repair_attempt_total = getattr(self, "tpsl_repair_attempt_total", 0) + 1
                    prot_req = ProtectionRequest(
                        symbol=symbol,
                    ...
                    self.tpsl_repair_success_total = getattr(self, "tpsl_repair_success_total", 0) + 1
                    ...
                    self.tpsl_repair_failure_total = getattr(self, "tpsl_repair_failure_total", 0) + 1
        ...
        _log.critical(
            ...
            symbol, getattr(self, "bot_instance_id", "unknown"), has_sl, has_tp, repair_source, sl_price, tp_price,
            extra={"bot_id": getattr(self, "bot_instance_id", "unknown"), "symbol": symbol, "repair_source": repair_source}
        )
