...
class AutoPilotAllocation(BaseModel):
    ...
    def validate_allocation_contract(cls, values):
        mode = values.get("allocation_type", "fixed_amount")
        total = values.get("total_capital_budget")
        per_position = values.get("trade_amount_per_position")
        if mode == "fixed_amount" and total is not None and per_position is not None and per_position > total:
            raise ValueError("trade_amount_per_position cannot exceed total_capital_budget")
        if mode == "percent_balance" and per_position is not None and not 0 < per_position <= 100:
...
def deploy_auto_pilot(
    ...
    try:
        ...
        instances = service.deploy_auto_pilot(
            ...
            allocation_type=request.allocation.allocation_type,
