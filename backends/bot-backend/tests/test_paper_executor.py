...
def test_paper_mode_close_requires_an_existing_internal_position():
    client = PriceOnlyClient(price=100.0)
    executor = BinanceExecutor(client=client, execution_mode="paper")
    ...
    # Phase 4 §48: the close contract now fails closed with an explicit reason
    # code instead of an ad-hoc message string.
    assert result.error == "PAPER_CLOSE_SIDE_UNKNOWN"
...
def test_paper_mode_close_uses_open_position_side_and_quantity():
    ...
    assert result.details["position_before"] == "BUY"
    assert result.details["filled_qty"] == pytest.approx(opened.details["filled_qty"])
    assert client.live_order_calls == []
