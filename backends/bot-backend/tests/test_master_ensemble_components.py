...
def test_crypto_bypasses_fixed_session_by_default(monkeypatch):
    ensemble = _ensemble(monkeypatch, Signal.BUY)
    monkeypatch.setattr(config_module.settings, "ENSEMBLE_SESSION_FILTER_ENABLED", True)
    monkeypatch.setattr(config_module.settings, "ENSEMBLE_SESSION_WINDOWS_UTC", "00:00-00:00")

    result = ensemble.get_signal("BTCUSDT", market_type="CRYPTO")

    assert result.reason != "SESSION_BLOCKED"
    assert result.meta["session_gate_result"] == "bypassed"
    assert result.meta["session_reason_code"] == "CRYPTO_SESSION_24_7_BYPASS"

    breakdown = build_hold_breakdown(
        symbol="BTCUSDT",
    ...
    assert breakdown["session_allowed"] is True
    assert breakdown["session_status"] == "CRYPTO_SESSION_24_7_BYPASS"


def test_forex_still_obeys_configured_session(monkeypatch):
    ensemble = _ensemble(monkeypatch, Signal.BUY)
    monkeypatch.setattr(config_module.settings, "ENSEMBLE_SESSION_FILTER_ENABLED", True)
    monkeypatch.setattr(config_module.settings, "ENSEMBLE_SESSION_WINDOWS_UTC", "00:00-00:00")

    result = ensemble.get_signal("EURUSD", market_type="FOREX")

    assert result.reason == "SESSION_BLOCKED"
    assert result.meta["session_reason_code"] == "SESSION_BLOCKED"


def test_crypto_can_explicitly_enforce_session(monkeypatch):
    ensemble = _ensemble(monkeypatch, Signal.BUY)
    monkeypatch.setattr(config_module.settings, "ENSEMBLE_SESSION_FILTER_ENABLED", True)
    monkeypatch.setattr(config_module.settings, "ENSEMBLE_SESSION_WINDOWS_UTC", "00:00-00:00")

    result = ensemble.get_signal("BTCUSDT", market_type="CRYPTO", enforce_session=True)

    assert result.reason == "SESSION_BLOCKED"


def test_active_crypto_bot_context_reaches_strategy_as_24_7(monkeypatch):
    instance = SimpleNamespace(
        id="bot_e5fe913972a9",
        user_id="b04f9242-39b4-48aa-915b-ef2096c4a00f",
        broker_account_id="brk_c729454e6c98",
        market_type="CRYPTO",
    ...
    context = BotRunContext.from_bot_instance(
        instance,
        {"api_key": "unused-in-unit-test", "environment": "stale-live"},
        account_environment="demo",
    )
    ensemble = _ensemble(monkeypatch, Signal.BUY)
    monkeypatch.setattr(config_module.settings, "ENSEMBLE_SESSION_FILTER_ENABLED", True)
    monkeypatch.setattr(config_module.settings, "ENSEMBLE_SESSION_WINDOWS_UTC", "06:00-19:00")

    results = [
        ensemble.get_signal(symbol, market_type=context.market_type)
        for symbol in context.symbols
    ]

    assert context.interval == "15m"
    assert context.broker_environment == "demo"
    assert all(result.reason != "SESSION_BLOCKED" for result in results)
    assert all(
        result.meta["session_reason_code"] == "CRYPTO_SESSION_24_7_BYPASS"
