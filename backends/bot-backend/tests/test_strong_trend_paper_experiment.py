...
def test_monitor_stop_recommendation_blocks_paper_experiment(tmp_path):
    report = tmp_path / "strong-trend-status.json"
    report.write_text('{"stop_recommended": true}', encoding="utf-8")
    status = evaluate_strong_trend_guard(config(
        STRONG_TREND_MONITOR_REPORT_PATH=str(report)
    ))
    assert status.monitoring_stop_recommended is True
    assert status.effective_unblocked is False
    assert status.reason == "monitoring_stop_recommended"
