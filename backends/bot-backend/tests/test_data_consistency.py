...
def _make_db(with_quarantine: bool = True) -> DB:
    ...
    with db.connect() as conn:
        # DB initializes the production-compatible schema. This fixture needs a
        # deliberately minimal historical schema, so replace only its local
        # in-memory copies before exercising compatibility behavior.
        conn.executescript("""
            DROP TABLE IF EXISTS broker_accounts;
            DROP TABLE IF EXISTS broker_credentials_v2;
