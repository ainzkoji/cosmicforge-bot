...
class TestKillSwitch:
    ...
    def _make_runner_stub(self, position_map: dict, close_raises: bool = False):
        ...
        runner.executor.execute_signal.return_value.success = False
        runner.executor.execute_signal.return_value.error = "simulated close failure"
        runner.position_manager = MagicMock()
        runner.position_manager.get_position.side_effect = lambda sym: MagicMock(
            current_qty=1.0, entry_price=100.0, position_id=f"pos-{sym}"
        )
        if close_raises:
