...
def test_record_fill_derives_user_id_from_bot_instance(db):
    ...
    with db.connect() as conn:
        row = conn.execute(
            "SELECT user_id, bot_instance_id, broker_account_id, run_id, cycle_id, trace_id, "
            "market_type, execution_mode FROM trade_fills "
            "WHERE position_id='pos-1' AND action='OPEN' LIMIT 1"
        ).fetchone()
