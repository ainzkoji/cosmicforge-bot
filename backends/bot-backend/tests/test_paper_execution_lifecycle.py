...
def _open_long(pm: PositionManager, executor: BinanceExecutor | None = None):
    ...
    if executor is not None:
        executor.paper_executor.seed_position("BTCUSDT", "LONG", 1.0, 100.0)
    return pos
...
def test_paper_stop_hit_can_close_without_live_order_calls():
    ...
    pm = PositionManager()
    _open_long(pm, executor)

    action = pm.update_price("BTCUSDT", 89.0, current_atr=2.0)
    close = executor.execute_signal("BTCUSDT", "CLOSE", 0.0, position_side="LONG", remaining_quantity=1.0)
...
def test_paper_tp1_runner_can_reach_tp2_without_live_order_calls():
    ...
    close = executor.execute_signal("BTCUSDT", "CLOSE", 0.0, position_side="LONG", remaining_quantity=0.5)
