...
class TraceRecorder:
    ...
    def record_candle(self, trace_id: str, *, open_time: int | None, close_time: int | None) -> None:
        with self._lock:
            trace = self._traces.get(trace_id)
            if trace:
                trace.candle_open_time = open_time
                trace.candle_close_time = close_time
        ...
        try:
            ...
            try:
                ...
                conn.execute(
                    ...
                    (
                        ...
                        signals_json, trace.chosen_strategy, trace.final_signal, trace.final_confidence, trace.reason_codes,
                         (1 if trace.gate_allowed else 0) if trace.gate_allowed is not None else None,
                         trace.gate_reason, json.dumps(trace.gate_details),
                ...
                canonical = {
                    ...
                    "component_signals": json.loads(signals_json),
                    "buy_score": trace.buy_score,
                    "sell_score": trace.sell_score,
                    "raw_confidence": trace.final_confidence,
                    "effective_entry_threshold": trace.threshold,
                    "risk_result": trace.gate_allowed,
                    "risk_reason": trace.gate_reason,
                    "sizing_result": trace.sizing,
                    "quantity": trace.final_qty or trace.fill_qty,
                    "ml_mode": trace.ml_action,
                    "ml_score": trace.ml_score,
                    "execution_attempted": trace.submit_attempted,
                    "fill_id": None,
                ...
                conn.execute(
                    ...
                    (
                        ...
                        trace.candle_close_time, trace.ts, trace.intended_action or trace.final_signal,
