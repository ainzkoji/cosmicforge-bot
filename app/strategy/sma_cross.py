    ...
    def get_signal(self, symbol: str) -> SignalResult:
        ...
        if sig == "BUY":
            ...
            if conf < self.min_confidence:
                return SignalResult(Signal.HOLD, 0.0, "gated")
            return SignalResult(Signal.BUY, conf, "sma_cross")

        if sig == "SELL":
            conf = 0.65
            if conf < self.min_confidence:
                return SignalResult(Signal.HOLD, 0.0, "gated")
            return SignalResult(Signal.SELL, conf, "sma_cross")

        return SignalResult(Signal.HOLD, 0.0, "no_cross")
